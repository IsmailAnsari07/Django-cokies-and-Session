...
def set_cookie(request):
    ...
    response.set_cookie('username','good coder' ) #max_age = 120 we use for how log cookie wil be visible
    return response
...
def create_session(request):
    request.session['name'] = 'good Coder'
...
def access_session(request):
    ...
    if request.session.get('name'):
        response += "Name is {0}".format(request.session.get('name'))
